#!/usr/bin/env python3
"""
[BACKTEST FIX] 현재 실전 봇 최신 방탄 로직 vs 기존 로직 비교
- 정확한 Unix Epoch ms 타임스탬프 변환
- 15배 레버리지 (Maker 0.02% / Taker 0.05%)
- 380ms 레이턴시 주입
- Dynamic ATR (1.0% ~ 2.0%) vs 기존 고정 1.0%
- Single Position Lock (체결 시 반대쪽 즉시 취소 & 갱신 올스톱)
- 3분 타임아웃 강제 탈출 (Time-based Exit)
- Post-TP 90s Cooldown & Post-SL 3m Circuit Breaker
- Volatility Shield 2.5%
"""
...
def simulate(prices: np.ndarray, ts_ms: np.ndarray, use_full_shield: bool = True):
    n = len(prices)
    if n < 1000:
        return None
        ...
        if in_position:
            ...
            if pos_side == "Long":
                ...
                # SL 손절 (-2.00%)
                    ...
                    in_position = False
                    if use_full_shield:
                        circuit_breaker_until = cur_ts + 180000  # 3m
                # 3분 타임아웃
        ...
        # 2. 무포지션 상태
            ...
            if not in_position and (cur_ts - last_refresh_ts >= 30000):
                ...
                if use_full_shield and (cur_ts < cooldown_until or cur_ts < circuit_breaker_until):
                    ...
                    if use_full_shield and cur_vol >= 2.50:
                        active_long_order = None
                        active_short_order = None
                    else:
...
def main():
    print("=" * 80, flush=True)
    print("🚀 [초정밀 백테스트] 현재 최신 방탄 그리드 (Dynamic ATR + 3분 타임아웃) vs 기존 고정 그리드", flush=True)
    print("조건: Bybit 15배 레버리지 | 380ms 레이턴시 주입 | Maker 0.02% / Taker 0.05% 수수료", flush=True)
    print("=" * 80, flush=True)
    ...
    for sym in symbols:
        try:
            ...
            print(f"\n🪙 심볼: {sym} (총 {len(df):,}개 틱 데이터)", flush=True)
            print("-" * 80, flush=True)

            res_old = simulate(prices, ts_ms, use_full_shield=False)
            if res_old:
                print(f"  [기존 단순형] 거래: {res_old['trades']:2d}회 | 승률: {res_old['win_rate']:5.1f}% | 총순익: {res_old['total_pnl']:+6.2f}% | 손익비(PF): {res_old['pf']:4.2f} | TP: {res_old['tp']}회, SL: {res_old['sl']}회 | 중앙보유: {res_old['median_hold']:.1f}초", flush=True)
            else:
                print("  [기존 단순형] 거래 없음", flush=True)

            res_new = simulate(prices, ts_ms, use_full_shield=True)
            if res_new:
                print(f"  [현재 방탄형] 거래: {res_new['trades']:2d}회 | 승률: {res_new['win_rate']:5.1f}% | 총순익: {res_new['total_pnl']:+6.2f}% | 손익비(PF): {res_new['pf']:4.2f} | TP: {res_new['tp']}회, SL: {res_new['sl']}회 (타임아웃익절: {res_new['to_tp']}회, 타임아웃손절: {res_new['to_sl']}회) | 중앙보유: {res_new['median_hold']:.1f}초", flush=True)
            else:
                print("  [현재 방탄형] 거래 없음", flush=True)
